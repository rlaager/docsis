  ...
  for (i = 0; i < NR_CMTS_MIC_TLVS; i++)
      ...
      while ((unsigned int) (cp - tlvbuf) < tlvbuflen)
	{
	  if (cp[0] == digest_order[i])
	    ...
	    {
	      if ( cp[0] == 64 ) {
		fprintf(stderr,"%s: warning: TLV64 (length > 255) not allowed in DOCSIS config files\n", prog_name);
...
usage (char *prog_name)
{
  ...
  fprintf(stderr,"To encode a cable modem configuration file from stdin to stdout: \n\t %s -E <key_file>\n", prog_name);
...
main (int argc, char *argv[])
{
  ...
  if (encode_docsis)
	...
	{
	  fprintf(stderr,"%s: error: can't open keyfile %s\n", prog_name, key_file);
	  exit (-5);
	}
	 		...
	 		unsigned char *key, unsigned int keylen, int encode_docsis )
{
  ...
  if (input_file!=NULL && output_file!=NULL) 
  {
      ...
      {
        fprintf(stderr,"%s: Error: source file is the same as destination file\n", prog_name);
        return -1;
      }
  }

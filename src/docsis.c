...
void usage(char *prog_name ) 
{
	...
	printf ("\nWhere:\n<modem_cfg_file>\t= name of text (human readable) configuration file\n<key_file>\t\t= text file containing the authentication key to be \n\t\t\t  used for the CMTS MIC (Message Integrity Check).\n<output_file> \t\t= name of output file where you want the binary data\n\t\t\t  to be written to (if it doesn't exist it is created).\n\t\t\t  This file can be TFTP-downloaded by DOCSIS-compliant\n\t\t\t  cable modems\n<binary_file>\t\t= name of binary file you to be decoded\n"); 
